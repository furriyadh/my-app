...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(
      `${baseUrl}/?error=oauth_error&message=${encodeURIComponent(errorMessage)}`
    );
  }
}
...
async function saveAccountsToDatabase(userId: string, accounts: {
  ...
  merchantCenterAccounts: MerchantCenterAccount[];
  youtubeChannels: YouTubeChannel[];
  analyticsAccounts: GoogleAnalyticsAccount[];
  businessLocations: GoogleMyBusinessLocation[];
}) {
  const { googleAdsAccounts, merchantCenterAccounts, youtubeChannels, analyticsAccounts, businessLocations } = accounts;
