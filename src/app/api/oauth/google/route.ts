...
// YouTube-only scopes - فقط ما هو مطلوب لـ YouTube API
const YOUTUBE_SCOPES = [
  'https://www.googleapis.com/auth/youtube.readonly',
  'openid',  // مطلوب للحصول على ID token
  'email'    // للحصول على email لربط الحساب
];

// Google Ads scopes - فقط ما هو مطلوب لـ Google Ads API
...
// Google Analytics scopes - فقط ما هو مطلوب لـ Analytics API
...
// Google Tag Manager scopes - فقط ما هو مطلوب لـ GTM API
const GTM_SCOPES = [
...
// Google Merchant Center scopes - فقط ما هو مطلوب لـ Content API
const MERCHANT_SCOPES = [
  'https://www.googleapis.com/auth/content',
  'openid',
  'email'
];

// Map scope parameter to actual scopes
const SCOPE_MAP: Record<string, string[]> = {
  'youtube': YOUTUBE_SCOPES,
  'ads': GOOGLE_ADS_SCOPES,
  'google-ads': GOOGLE_ADS_SCOPES,
  'analytics': ANALYTICS_SCOPES,
  'gtm': GTM_SCOPES,
  'tagmanager': GTM_SCOPES,
  'merchant': MERCHANT_SCOPES,
  'content': MERCHANT_SCOPES,
  'all': GOOGLE_OAUTH_SCOPES
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    const scopesToUse = scopeParam && SCOPE_MAP[scopeParam] ? SCOPE_MAP[scopeParam] : GOOGLE_OAUTH_SCOPES;
    ...
    authUrl.searchParams.set('include_granted_scopes', 'false'); // false = يطلب فقط الأذونات المحددة
