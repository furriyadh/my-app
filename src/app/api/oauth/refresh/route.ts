...
// دالة مساعدة للتحقق من صحة الـ token
export async function validateAndRefreshToken(userId: string, provider: string = 'google'): Promise<string | null> {
  try {
    ...
    if (expiresAt && expiresAt > now) {
      // الـ token ما زال صالحاً
      return tokenData.access_token;
    }

    // الـ token منتهي الصلاحية، نحتاج لتحديثه
    if (!tokenData.refresh_token) {
      return null;
    }
