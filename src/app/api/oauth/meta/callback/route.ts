...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        let redirectAfter = '/integrations/meta-ads';
        if (savedState && stateParam) {
            try {
                ...
                if (savedStateData.state !== receivedStateData.state) {
                    ...
                    return NextResponse.redirect(
                        `${baseUrl}/integrations?error=state_mismatch`
                    );
                }
        ...
        let userInfo = { id: '', name: '', email: '', picture: '' };
        if (userResponse.ok) {
        ...
        return NextResponse.redirect(
            `${baseUrl}/integrations?error=${encodeURIComponent('OAuth failed')}`
        );
    }
}
