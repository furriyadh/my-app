        ...
        {status === 'connected' ? (
          <div className="flex gap-3">
            <button
              ...
              onClick={() => window.location.href = `/dashboard/google-ads/integrations/${integrationId}`}
            >
...
const IntegrationsPage: React.FC = () => {
  ...
  const handleConnect = async (integrationId: string) => {
    ...
    const openOAuthPopup = (url: string) => {
      ...
      // ✅ Listen for success message from popup (window.postMessage)
      ...
      // ✅ Listen for success message from BroadcastChannel (More reliable)
      ...
      const handleSuccess = (data: any) => {
        ...
        // Navigate or reload
        if (data.redirectUrl && data.redirectUrl !== window.location.pathname) {
      ...
      // Poll as fallback for popup close
    ...
    if (integrationId === 'google-ads') {
      openOAuthPopup('/api/oauth/google?redirect_after=' + encodeURIComponent('/dashboard/google-ads/integrations/google-ads') + '&scope=ads');
    } else if (integrationId === 'google-analytics') {
      openOAuthPopup('/api/oauth/google?redirect_after=' + encodeURIComponent('/dashboard/google-ads/integrations/google-analytics') + '&scope=analytics');
    } else if (integrationId === 'google-tag-manager') {
      openOAuthPopup('/api/oauth/google?redirect_after=' + encodeURIComponent('/dashboard/google-ads/integrations/google-tag-manager') + '&scope=gtm');
    } else if (integrationId === 'google-merchant') {
      openOAuthPopup('/api/oauth/google?redirect_after=' + encodeURIComponent('/dashboard/google-ads/integrations/google-merchant') + '&scope=merchant');
    } else if (integrationId === 'youtube-channel') {
      openOAuthPopup('/api/oauth/google?redirect_after=' + encodeURIComponent('/dashboard/google-ads/integrations/youtube-channel') + '&scope=youtube');
    } else if (integrationId === 'meta-ads') {
      openOAuthPopup('/api/oauth/meta?redirect_after=' + encodeURIComponent('/dashboard/google-ads/integrations/meta-ads'));
    } else {
