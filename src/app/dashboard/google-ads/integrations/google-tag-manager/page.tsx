...
const styles = `
  ...
  .container-card::after {
    ...
    background: radial-gradient(
      ...
      rgba(59, 130, 246, calc(var(--glow-intensity) * 0.35)) 0%,
      rgba(59, 130, 246, calc(var(--glow-intensity) * 0.2)) 25%,
      rgba(59, 130, 246, calc(var(--glow-intensity) * 0.1)) 45%,
      rgba(59, 130, 246, calc(var(--glow-intensity) * 0.05)) 60%,
      transparent 80%
    );
    pointer-events: none;
    z-index: 1;
...
const GoogleTagManagerContent: React.FC = () => {
    const router = useRouter();
    const searchParams = useSearchParams();
    const [loading, setLoading] = useState(true);
    const [containers, setContainers] = useState<GTMContainer[]>([]);
    const [linkedContainers, setLinkedContainers] = useState<LinkedContainer[]>([]);
    ...
    const fetchLinkedContainers = async () => {
        try {
            ...
            if (data.success && data.containers) {
                setLinkedContainers(data.containers);
                console.log('📋 Linked containers:', data.containers);
            }
        } catch (err) {
            console.error('Error fetching linked containers:', err);
        }
    };
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.containers?.length === 0) {
                    setError('No containers found in your account. Make sure you have a GTM Container.');
                }
            } else {
                setError(data.message || 'Failed to fetch containers');
            }
        } catch (err) {
    ...
    // Check if a container is already linked
    const isContainerLinked = (containerPath: string): boolean => {
        return linkedContainers.some(lc => lc.container_id === containerPath);
    };
    ...
    const handleSave = async () => {
        ...
        if (isContainerLinked(selectedContainer)) {
            const cont = containers.find(c => c.path === selectedContainer);
            setAlreadyLinkedNotification({
                show: true,
                containerName: cont?.name || selectedContainer,
        ...
        try {
            ...
            const response = await authFetch('/api/gtm/connected', {
                ...
                body: JSON.stringify({
                    accountId: selectedCont?.accountId || null,
                    accountName: selectedCont?.accountName || null,
                    containerId: selectedContainer,
                    containerName: selectedCont?.name || null,
                    containerPublicId: selectedCont?.publicId || null,
                    usageContext: selectedCont?.usageContext || null,
                }),
            });
            ...
            if (result.success) {
                ...
                if (result.alreadyExists) {
                    setAlreadyLinkedNotification({
                        show: true,
                        containerName: selectedCont?.name || selectedContainer,
                        containerId: selectedContainer
                    });
                    return;
                }
                ...
                const adminResponse = await authFetch('/api/gtm/add-admin', {
                    ...
                    body: JSON.stringify({
                        accountPath: `accounts/${selectedCont?.accountId}`,
                    }),
                });
                ...
                localStorage.setItem('selected_gtm_container', selectedContainer);
                router.push('/dashboard/google-ads/integrations');
            } else {
                alert(result.message || 'Failed to save container');
            }
    ...
    const GTMIcon = () => (
        <svg viewBox="0 0 192 192" className="w-6 h-6">
            <path fill="#8AB4F8" d="M111.31 176.79L78.14 143.62l33.17-33.17 33.17 33.17z" />
            <path fill="#4285F4" d="M78.14 143.62L44.97 110.45l66.34-66.34 33.17 33.17z" />
            <path fill="#246FDB" d="M144.48 110.45L111.31 77.28l33.17-33.17 33.17 33.17z" />
            <path fill="#8AB4F8" d="M111.31 176.79a23.45 23.45 0 1 0 0-46.9 23.45 23.45 0 0 0 0 46.9z" />
        </svg>
    );
                ...
                <div className="container mx-auto px-4 py-8">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-400 text-sm">
                            Select a container to connect to your dashboard
                        </p>
                    </div>
                        ...
                        <div className="max-w-md mx-auto mb-6">
                            ...
                            >
                                ...
                                <div className="relative mb-4">
                                    <p className="text-gray-400 text-sm">
                                        Container <span className="font-semibold text-blue-300">{alreadyLinkedNotification.containerName}</span> is already linked to your account.
                                    </p>
                                </div>
                    ...
                    <div className="max-w-2xl mx-auto">
                        {loading ? (
                            <div className="text-center py-16">
                                ...
                                <p className="text-gray-400">Searching for containers...</p>
                            </div>
                        ) : error ? (
                            <div className="text-center py-16">
                                ...
                                <GlowButton
                                    onClick={() => router.push('/dashboard/google-ads/integrations')}
                                    variant="blue"
                                >
                            ...
                            <>
                                ...
                                >
                                    {/* Outer container with blue glow */}
                                    <div className="relative rounded-[28px] p-[2px] bg-gradient-to-br from-blue-500/60 via-blue-400/40 to-blue-600/60 shadow-2xl shadow-blue-500/30 transition-all duration-500 hover:shadow-blue-500/50 overflow-hidden">

                                        {/* Inner container - dark blue background */}
                                        <div className="relative rounded-[26px] bg-[#000810] backdrop-blur-xl p-5 overflow-hidden">
                                            ...
                                            <div className="absolute top-1/3 right-1/4 w-1.5 h-1.5 bg-blue-300/40 rounded-full animate-float-slow"></div>
                                            <div className="absolute bottom-1/4 left-1/4 w-1 h-1 bg-blue-400/40 rounded-full animate-float-fast"></div>
                                            <div className="absolute bottom-10 right-8 w-1.5 h-1.5 bg-blue-400/50 rounded-full animate-float-medium"></div>
                                            ...
                                            {/* Containers list - scrollable with max height */}
                                            <div className="space-y-3 relative z-10 max-h-[50vh] sm:max-h-[55vh] md:max-h-[60vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-blue-500/30 scrollbar-track-transparent">
                                                {containers.map((container) => {
                                                    const isSelected = selectedContainer === container.path;
                                                    const isLinked = isContainerLinked(container.path);

                                                    return (
                                                        <div
                                                            key={container.path}
                                                            onClick={() => setSelectedContainer(container.path)}
                                                            className={`container-item cursor-pointer rounded-xl p-4 border transition-all duration-300 ${isSelected
                                                                ? 'bg-green-500/10 border-green-500/50'
                                                                : isLinked
                                                                    ? 'bg-emerald-500/10 border-emerald-500/30'
                                                                    : 'bg-[#000d1a] border-blue-500/20 hover:bg-[#001025] hover:border-blue-500/40'
                                                                }`}
                                                        >
                                                                ...
                                                                <div className="flex items-center gap-3 min-w-0">
                                                                    <div className={`w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center ${isSelected
                                                                        ...
                                                                        : isLinked
                                                                            ...
                                                                            : 'bg-[#000510] border border-blue-500/20'
                                                                        }`}>
                                                                        <GTMIcon />
                                                                    </div>
                                ...
                                <div className="mt-8 flex justify-center">
                                    <GlowButton
                                        onClick={handleSave}
                                        disabled={!selectedContainer || saving}
                                        variant="blue"
                                    >
...
export default function GoogleTagManagerSetupPage() {
    return (
        <Suspense fallback={
