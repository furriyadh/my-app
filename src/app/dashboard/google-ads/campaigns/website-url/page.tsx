...
const WebsiteUrlPage: React.FC = () => {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (nextCampaignType === 'VIDEO' && selectedVideos.length > 0) {
        ...
        router.push('/dashboard/google-ads/campaigns/video-subtype');
      } else {
        // Skip campaign type selection - type is already auto-detected from URL
        router.push('/dashboard/google-ads/campaigns/location-targeting');
      }
  ...
  const handleBack = () => {
    router.push('/dashboard/google-ads');
  };
                        ...
                        <div className="!w-full">
                          <div className="mt-4 p-5 bg-white/10 border border-cyan-400/30 rounded-xl backdrop-blur-sm">
                            ...
                            {isFetchingMerchants ? (
                              ...
                              <div className="max-h-60 overflow-y-auto">
                                {merchantAccounts.map((account, index) => (
                                  <div
                                    ...
                                    className={`grid grid-cols-12 gap-2 px-3 py-3 transition-all items-center ${index % 2 === 0 ? 'bg-gray-50 dark:bg-white/5' : 'bg-transparent'
                                      } ${selectedMerchant === account.id
                                        ? 'bg-cyan-500/20 ring-1 ring-cyan-400/50'
                                        : account.linked ? 'hover:bg-gray-100 dark:hover:bg-white/10 cursor-pointer' : 'opacity-70'
                                      }`}
                                  >
                                    {/* Merchant Info */}
                                    <div className="col-span-5 flex items-center gap-3">
                                      <div className="w-9 h-9 bg-gradient-to-br from-cyan-100 to-blue-100 dark:from-cyan-500/30 dark:to-blue-500/30 rounded-lg flex items-center justify-center flex-shrink-0">
                                        <ShoppingCart className="w-4 h-4 text-cyan-600 dark:text-cyan-300" />
                                      </div>
                                      <div className="min-w-0">
                                        <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{account.name}</p>
                                        <p className="text-xs text-gray-500 dark:text-white/50">{account.id}</p>
                                      </div>
                                    </div>
                                    {/* Products Count */}
                                    <div className="col-span-2 text-center text-sm text-gray-700 dark:text-white/70">
                                        ...
                                        <button
                                          onClick={(e) => {
                                            ...
                                            localStorage.setItem('campaignData', JSON.stringify({
                                              ...
                                              returnUrl: '/dashboard/google-ads/campaigns/website-url'
                                            }));
                                            // Redirect to merchant linking page
                                            window.location.href = '/dashboard/google-ads/integrations';
                                          }}
                                          className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 dark:bg-blue-500/30 hover:bg-blue-200 dark:hover:bg-blue-500/50 text-blue-700 dark:text-blue-300 text-xs font-medium rounded-full transition-colors"
                                        >
                              ...
                              <div className="text-center py-8 text-gray-500 dark:text-white/60 text-sm">
                                <p>{language === 'ar' ? 'لا توجد حسابات Merchant Center متاحة' : 'No Merchant Center accounts available'}</p>
                                <button
                                  onClick={() => window.location.href = '/dashboard/google-ads/integrations'}
                                  className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-cyan-100 dark:bg-cyan-500/30 hover:bg-cyan-200 dark:hover:bg-cyan-500/50 text-cyan-700 dark:text-cyan-300 text-sm font-medium rounded-lg transition-colors"
                                >
                              ...
                              <>
                                ...
                                <div className="relative mb-4">
                                  ...
                                  <input
                                    ...
                                    className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-white/10 border border-gray-200 dark:border-white/20 rounded-lg text-gray-900 dark:text-white text-sm placeholder-gray-400 dark:placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-red-400/50"
                                  />
                                  {isVideoSearching && (
                                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                      ...
                                      return (
                                        <div
                                          ...
                                          className={`p-2 rounded-lg cursor-pointer transition-all flex items-start gap-3 ${isSelected
                                            ? 'bg-red-50 dark:bg-red-500/30 border border-red-200 dark:border-red-400/70'
                                            : 'bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-200 dark:hover:border-white/20'
                                            }`}
                                        >
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <p className="text-xs font-medium text-gray-900 dark:text-white line-clamp-2 leading-snug mb-1" title={video.title}>
                                              {video.title}
                                            </p>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="relative country-dropdown-container">
                        <div
                          ...
                          className="w-24 px-4 py-5 bg-gray-50 dark:bg-gray-900 backdrop-blur-sm border-2 border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white cursor-pointer flex items-center justify-center hover:border-blue-500/50 dark:hover:border-blue-400/50 transition-all"
                        >
                          <div className="flex items-center gap-2">
                        ...
                        {/* Dropdown Menu */}
                        {isCountryDropdownOpen && (
                          <div className="country-dropdown-container absolute z-[9999] top-full mt-2 w-72 bg-white dark:bg-gray-800 backdrop-blur-lg rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                            {/* Search Input */}
                            <div className="p-3 border-b border-gray-200 dark:border-gray-700">
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  className="country-search-input w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-gray-900 backdrop-blur-sm border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50"
                                  autoFocus
                                />
                              </div>
                            </div>
                              ...
                              {filteredCountries.length > 0 ? (
                                filteredCountries.map((country) => (
                                  <div
                                    ...
                                    className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer flex items-center justify-between group transition-colors"
                                  >
                                    <div className="flex items-center gap-2">
                                      <ReactCountryFlag
                                    ...
                                    <span className="text-xs font-medium text-gray-400 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white">
                                      {country.code}
                                    </span>
                                  </div>
                                ))
                      ...
                      <div className="flex-1">
                        <input
                          ...
                          className="w-full px-4 py-5 bg-gray-50 dark:bg-gray-900 backdrop-blur-sm border-2 border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 text-sm transition-all duration-200"
                        />
                      </div>
                    </div>
                  </div>
