...
const variants = {
  ...
  purple: {
    light: {
      ...
      blobHighlight: "#a78bfa",
      blobShadow: "rgba(147, 51, 234, 0.25)",
      innerGlow: "rgba(147, 51, 234, 0.1)",
      innerHighlight: "rgba(221, 214, 254, 0.15)",
      outerBg: "linear-gradient(135deg, #faf5ff 0%, #ede9fe 100%)",
      innerBg: "linear-gradient(135deg, #ffffff 0%, #faf5ff 100%)",
      textColor: "#4c1d95",
    },
    dark: {
      outerGlow: "rgba(233, 213, 255, 0.4)",
      blobGlow: "rgba(138, 43, 226, 0.5)",
      blobHighlight: "#b19cd9",
...
const GlowButton: React.FC<GlowButtonProps> = ({
  ...
  // Always use dark theme colors for consistent button appearance
  const variantColors = variants[variant] || variants.blue;
  const colors = variantColors.dark;
  ...
  return (
    <button
      ...
      style={{
        background: disabled ? '#e5e7eb' : colors.outerBg,
      }}
    >
      {!disabled && (
        <>
      ...
      <div
        ...
        style={{
          background: disabled ? '#f3f4f6' : colors.innerBg,
