...
const parseColor = (color: string): THREE.Color => {
  try {
    ...
    console.log(`Parsed THREE.Color:`, threeColor);
    return threeColor;
  } catch (error) {
    ...
    if (color.startsWith("rgb")) {
      ...
      if (matches && matches.length >= 3) {
        ...
        console.log(`RGB converted: r=${r}, g=${g}, b=${b}`);
        return new THREE.Color(r, g, b);
      }
    }
    console.warn(`Could not parse color: ${color}. Falling back to blue.`);
    return new THREE.Color(0x3B82F6); // Default to blue instead of white
  }
};
...
const AnimatedWave: React.FC<AnimatedWaveProps> = ({
    ...
    if (autoDetectBackground && containerRef.current) {
      ...
      if (detectedBg) {
        ...
        if (isColorDark(parsedBgColor)) {
          ...
          return new THREE.Color(0x3B82F6); // Blue wave for light background
        }
      }
    }

    // Default to blue wave for our dashboard
    return new THREE.Color(0x3B82F6); // Default to blue wave
  }, [waveColor]); // إزالة autoDetectBackground من dependencies
  ...
  const createGroundPlain = useCallback(() => {
    ...
    const groundPlain = {
      ...
      create: function (scene: THREE.Scene) {
        ...
        console.log('Three.js Material color:', waveColorValue);
  ...
  const setupScene = useCallback(() => {
    ...
    fov,
    mouseInteraction,
  ]); // إزالة determineWaveColor و createGroundPlain من dependencies
  ...
  }, [wireframe, opacity]); // إزالة determineWaveColor من dependencies
