      ...
      <style>
        {`
          ...
          .card::before {
            ...
            inset: 0;
            border-radius: inherit;
            background: linear-gradient(145deg, rgba(132, 0, 255, 0.03), rgba(132, 0, 255, 0.01));
            pointer-events: none;
            z-index: 0;
          }
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-4">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-6 rounded-[24px] border-2 border-solid font-light overflow-hidden transition-all duration-300 ease-in-out ${
              ...
              return (
                ...
                >
                  <div className="card__header flex justify-between gap-3 relative text-white z-10">
                    <span className="card__label text-sm font-medium opacity-80 tracking-wide uppercase">{card.label}</span>
                    {Icon && <Icon className="w-6 h-6 opacity-70 text-purple-400" />}
                  </div>
                  <div className="card__content flex flex-col relative text-white z-10">
                    {card.value && (
                      <div className="text-5xl font-black mb-3 bg-gradient-to-r from-purple-400 via-pink-400 to-purple-400 bg-clip-text text-transparent leading-tight">
                        {card.value}
                      </div>
                    )}
